...
class Pdfresult {
    ...
    public static function setTime(&$startTime, &$endTime, &$during, $data) {
        ...
        $during = Numeric::convert($diff->h, 2) . ':' . (($diff->h == 0 && $diff->i == 0) ? '00' : Numeric::convert($diff->i, 2)) . ':' . Numeric::convert($diff->s, 2); // . ':00';
        if ($data['sh'] > 12) {
            $startTime = ($data['sh'] - 12) . ':' . Numeric::convert($data['sm'], 2) . ' PM';
        } else {
            $startTime = $data['sh'] . ':' . Numeric::convert($data['sm'], 2) . ' AM';
        }
        if ($data['eh'] > 12) {
            $endTime = ($data['eh'] - 12) . ':' . Numeric::convert($data['em'], 2) . ' PM';
        } else {
            $endTime = $data['eh'] . ':' . Numeric::convert($data['em'], 2) . ' AM';
        }
    }
